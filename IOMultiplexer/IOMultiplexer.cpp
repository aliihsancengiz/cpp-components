...
namespace io_multiplexer
{
    ...
    {
        mLogger->trace("Registering Event Fd : {} , Event Type : {}", ioObj.fd, IOEventTypeToString(type));
    ...
    {
        mLogger->trace("Deregistering Event Fd : {} ,Event Type : {}", ioObj.fd, IOEventTypeToString(type));
        ...
        while (runFlag)
        {
            ...
            for (int n = 0; n < nfds; ++n)
            {
                ...
                if (mEventHandlerMap.find(fd) != mEventHandlerMap.end() && mEventHandlerMap[fd].find(type) != mEventHandlerMap[fd].end() && (mEventHandlerMap[fd][type] != nullptr))
                {
                    try
                    {
                        mLogger->trace(" Got an Event on File Descriptor : {} , Event  : {} ", fd, IOEventTypeToString(type));
